...
def direction_of_incidence(audioBFormat_stft,f,t,p0,c):  
    ...
    return DOA,r,azimuth,elevation
...
def Diffuseness(audioBFormat_stft,f,t,dt,p0,c):
...
def plot_spectrogram(title, x, colorMap, xlabel, ylabel, barlabel, minValue, maxValue):
    plt.figure()
    plt.suptitle(title)
...
def Mask(data, diffuseness, threshold):
    mask = np.empty(np.shape(data))
    
    for x in range(np.shape(data)[0]):
        for y in range(np.shape(data)[1]):
            if (diffuseness[x,y]) <= threshold:
                mask[x,y] = 1
            else:
                mask[x,y] = np.nan
    return mask
     
def plot_reflection(azi, ele, diffuseness, threshold, title, xlabel, ylabel, barLabel):
    
    mask = Mask(azi, diffuseness, threshold)
       
    i=0
    azimuth = np.empty(1)
    elevation = np.empty(1)
    for x in range(azi[:,1].size):
        for y in range(azi[1,:].size):
            if mask[x,y] == 1: 
                if i == 0:
                    azimuth[i] = azi[x,y]
                    elevation[i] = ele[x,y]                    
    ...
    H, xedges, yedges = np.histogram2d(azimuth,elevation,bins=[360, 180])
    H = np.rot90(H)
    H = np.flipud(H)   
